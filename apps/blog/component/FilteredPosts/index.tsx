...
export const FilteredPosts = ({ allPosts }: FilteredPostsProps) => {
  ...
  const filteredPosts = useMemo(() => {
    return tag 
      ? allPosts.filter((post) => post.frontMatter.tags.some((t) => t.trim() === tag.trim()))
      : allPosts
  }, [allPosts, tag])
  ...
  return (
    <>
      ...
      <InfiniteScrollPosts key={tag || 'all'} initialPosts={initialPosts} initialHasMore={hasMore} tag={tag || undefined} />
    </>
  )
}
